...
CAR_WIDTH = 14     
CAR_HEIGHT = 8   
SENSOR_DIST = 15 # FIX ME! Distance sensors look ahead (pixels) - Currently unrealistic!
SENSOR_ANGLE = 5    # FIX ME! Angle spread of sensors (degrees) - Too narrow!
SPEED = 2.0#2          # FIX ME! Forward speed (pixels/step) - Way too fast!
TURN_SPEED = 1.0    # FIX ME! Regular turn angle (degrees/step) - Too slow!
SHARP_TURN = 18      # FIX ME! Sharp turn angle for tight corners (degrees) - Too small!
...
LR = 0.00005            # FIX ME! Learning rate for neural network optimizer
                    # Way too high - will cause instability!
                    # Hint: Usually 0.0001 to 0.01

TAU = 0.01 #0.005           # FIX ME! Polyak averaging coefficient for soft target updates
...
EPSILON_MIN = 0.001 # 0.01 # EXPLORATION RATE MINIMUM 0.001 -> 0.01 MODIFIED EXTRA
...
class DrivingDQN(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.net = nn.Sequential(
            ...
            nn.Linear(128, 256),
            nn.ReLU(),
            nn.Linear(256, 512),# INCREASED FROM 256 -> 512 MODIFIED EXTRA
            nn.ReLU(),
            nn.Linear(512, 256),# DECREASED FROM 256 -> 128 MODIFIED EXTRA
            nn.ReLU(),
            nn.Linear(256, 128),
            nn.ReLU(),
...
class CarBrain:
    def __init__(self, map_image: QImage):
        ...
        self.memory = deque(maxlen=20000) # MEMORY SIZE INCREASED FROM 10000 -> 50000 MODIFIED EXTRA
        
        # Prioritized Replay: separate buffer for high-reward episodes
        self.priority_memory = deque(maxlen=10000)  # Store successful episodes INCREASED FROM 3000 -> 10000 MODIFIED EXTRA
        self.current_episode_buffer = []  # Temporary buffer for current episode
        self.episode_scores = deque(maxlen=200)  # Track recent episode scores INCREASED FROM 100 -> 200 MODIFIED EXTRA
        ...
        self.steps = 0
        self.epsilon = 1.00  # FIX ME! Exploration rate - starts too low! 
    ...
    def get_state(self):
        ...
        for a in angles:
            ...
            if 0 <= sx < self.w and 0 <= sy < self.h:
                ...
                val = (brightness / 255.0) ** 0.8  # Non-linear scali#ng#
        ...
        # norm_dist = min(dist / 1200.0, 1.0) # DISTANCE NORMALIZATION 800 -> 1200 MODIFIED EXTRA
    ...
    def step(self, action):
        ...
        reward = -0.05 # REDUCED FROM -0.1 -> -0.05 MODIFIED EXTRA  Reward step penalty
        done = False
        
        car_center_val = self.check_pixel(self.car_pos.x(), self.car_pos.y())
        
        if car_center_val < 0.4:# Full crash penalty
            reward = -300 # # Increase from -200 -> -150 MODIFIED EXTRA
            done = True# Full crash penalty
            self.alive = False
        elif car_center_val < 0.5:
            reward = -150 # Increase from -100 -> -150 MODIFIED EXTRA Warning penalty (near boundary)
            done = True# Medium crash penalty
            self.alive = False
        elif car_center_val < 0.6:
            reward = -75
            done = True# Low crash penalty Small warning
            self.alive = False
        elif dist < 25: # REDUCED FROM 20 -> 25 MODIFIED EXTRA
            reward = 200 # Increase from 100 -> 200 MODIFIED EXTRA
            ...
            if self.prev_dist is not None and dist > self.prev_dist:
                reward -= 8 #10  # Penalty for moving away from target MODIFIED EXTRA
            self.prev_dist = dist
    ...
    def optimize(self):
        ...
        if len(batch) < BATCH_SIZE // 2: # REDUCED FROM BATCH_SIZE // 2 -> BATCH_SIZE // 4 MODIFIED EXTRA
            return 0
        ...
        # Clip Q-values to prevent explosion (fixes high Q-Score)
        next_q = torch.clamp(next_q, -200, 200) # MODIFIED EXTRA
        ...
        loss.backward()
        torch.nn.utils.clip_grad_norm_(self.policy_net.parameters(), max_norm=0.5) # GRADIENT CLIPPING MODIFIED EXTRA
        self.optimizer.step()
        
        if self.epsilon > EPSILON_MIN : self.epsilon *= 0.9995 # EXPLORATION RATE DECAY 0.01 -> 0.05 MODIFIED EXTRA
        return loss.item()
    ...
    def finalize_episode(self, episode_reward):
        ...
        # Auto-save best model based on episode reward
        if episode_reward > self.best_score:
            self.best_score = episode_reward
            try:
                self.save_weights(self.best_model_path)
            except Exception as e:
                # Silent fail to avoid crashing the app on disk issues
                print(f"[WARN] Failed to auto-save best model: {e}")

    def save_weights(self, path: str):
        """
        Save current policy/target networks and optimizer state.
        """
        state = {
    ...
    def load_weights(self, path: str) -> bool:
        """
        Load policy/target networks and optimizer state if file exists.
        Returns True on success, False otherwise.
        """
        if not os.path.exists(path):
            return False
        try:
            # Use weights_only=False to support older checkpoints that rely on pickled objects.
            # Only do this if you trust the source of the checkpoint.
            state = torch.load(path, map_location="cpu", weights_only=False)
        except Exception as e:
            print(f"[WARN] Failed to load weights from {path}: {e}")
            return False
        self.policy_net.load_state_dict(state.get("policy_net", {}))
        self.target_net.load_state_dict(state.get("target_net", {}))
        if "optimizer" in state:
            try:
                self.optimizer.load_state_dict(state["optimizer"])
            except Exception:
                # Optimizer shape mismatch is non-fatal; ignore
                pass
...
class NeuralNavApp(QMainWindow):
    def __init__(self):
        ...
        sf_layout.addWidget(QLabel("Q-Score:"), 2,0)
        sf_layout.addWidget(self.val_qscore, 2,1)
        
        self.val_loss = QLabel("0.00")
        self.val_loss.setStyleSheet(f"color: {C_ACCENT.name()}; font-weight: bold;")
        sf_layout.addWidget(QLabel("Loss:"), 3,0)
        sf_layout.addWidget(self.val_loss, 3,1)
        
        self.val_success = QLabel("0 (0%)")
        self.val_success.setStyleSheet(f"color: {C_ACCENT.name()}; font-weight: bold;")
        sf_layout.addWidget(QLabel("Success:"), 4,0)
        sf_layout.addWidget(self.val_success, 4,1)
        
        self.val_time = QLabel("0s")
        self.val_time.setStyleSheet(f"color: {C_ACCENT.name()}; font-weight: bold;")
        sf_layout.addWidget(QLabel("Overall Time:"), 5,0)
        sf_layout.addWidget(self.val_time, 5,1)
        
        self.val_steps = QLabel("0")
        self.val_steps.setStyleSheet(f"color: {C_ACCENT.name()}; font-weight: bold;")
        sf_layout.addWidget(QLabel("Overall Steps:"), 6,0)
        sf_layout.addWidget(self.val_steps, 6,1)
        
        self.val_best_score = QLabel("-∞")
        self.val_best_score.setStyleSheet(f"color: {C_SUCCESS.name()}; font-weight: bold;")
        sf_layout.addWidget(QLabel("Best Score:"), 7,0)
        sf_layout.addWidget(self.val_best_score, 7,1)
        ...
        # Logic
        self.setup_map("city_map.jpg") 
        if os.path.exists("best_model.pth"):
            self.brain.load_weights("best_model.pth")
            self.log("✅ Loaded best model weights")
        self.setup_state = 0 
    ...
    def save_best_weights(self):
        """
        Save current best weights to a user-chosen file.
        By default, suggests the internally tracked best-model path.
        """
        default_name = getattr(self.brain, "best_model_path", "best_model.pth")
        path, _ = QFileDialog.getSaveFileName(self, "Save Best Weights", default_name, "PyTorch Weights (*.pth)")
        if path:
            try:
                self.brain.save_weights(path)
                self.log(f"<font color='#A3BE8C'>✅ Weights saved to: {path}</font>")
            except Exception as e:
                self.log(f"<font color='#BF616A'>⚠️ Failed to save weights: {e}</font>")

    def load_weights(self):
        """
        Load weights from a .pth file and apply them to the current brain.
        """
        path, _ = QFileDialog.getOpenFileName(self, "Load Weights", "", "PyTorch Weights (*.pth)")
        if path:
            ok = self.brain.load_weights(path)
            if ok:
                self.log(f"<font color='#A3BE8C'>✅ Weights loaded from: {path}</font>")
            else:
                self.log(f"<font color='#BF616A'>⚠️ Could not load weights from: {path}</font>")
    ...
    def full_reset(self):
        ...
        self.current_loss = 0.0
        self.current_qscore = 0.0
        self.val_qscore.setText("0.00")
        self.val_loss.setText("0.00")
        self.val_success.setText("0 (0%)")
        self.val_time.setText("0s")
        self.val_steps.setText("0")
        self.val_best_score.setText("-∞")
    ...
    def game_loop(self):
        ...
        # Compute Q-score from current state
        with torch.no_grad():
            q_values = self.brain.policy_net(torch.FloatTensor(state).unsqueeze(0))
            self.current_qscore = q_values.max().item()
        ...
        if done:
            ...
            # Track successful episodes (episodes where car reached target)
            if self.brain.alive:
        ...
        self.val_qscore.setText(f"{self.current_qscore:.2f}")
        
        # Update loss display
        self.val_loss.setText(f"{self.current_loss:.4f}")
        
        # Update success display
        success_rate = (self.successful_episodes / self.total_episodes * 100) if self.total_episodes > 0 else 0
        self.val_success.setText(f"{self.successful_episodes} ({success_rate:.1f}%)")
        
        # Update overall time display
        if self.start_time is not None:
            elapsed_time = time.time() - self.start_time
            if elapsed_time < 60:
                self.val_time.setText(f"{elapsed_time:.1f}s")
            elif elapsed_time < 3600:
                minutes = int(elapsed_time // 60)
                seconds = int(elapsed_time % 60)
                self.val_time.setText(f"{minutes}m {seconds}s")
            else:
                hours = int(elapsed_time // 3600)
                minutes = int((elapsed_time % 3600) // 60)
                self.val_time.setText(f"{hours}h {minutes}m")
        
        # Update overall steps display
        self.val_steps.setText(f"{self.brain.steps}")
        
        # Update best score display
        if self.brain.best_score == -float("inf"):
            self.val_best_score.setText("-∞")
        else:
